# 移除时空上下文编码：移除PatchEmbedding_time模块。
import math
import numpy as np
import torch
...
class STEncoderBlock(nn.Module):
    ...
    def forward(self, x, dec_in, enc_out, TH, TP, adj, geo_mask=None, sem_mask=None):
        ...
        if TH is None or TP is None:
            # 如果 TH 或 TP 为 None，跳过时空注意力机制
            attn_output = x_nor1
        else:
            attn_output = self.drop_path(self.st_attn(x_nor1, x_nor1, x_nor1, TH, TP, adj, geo_mask=geo_mask, sem_mask=sem_mask))
        x = x + attn_output
        x_nor2 = self.norm2(x)
        x = x + self.drop_path(self.mlp(x_nor2))
        return x


class NewCity(nn.Module):
    def __init__(self, args, dataset_use, device, dim_in):
        super(NewCity, self).__init__()
        ...
        self.patch_embedding_time = None
    ...
    def forward(self, input, lbls, select_dataset):
        ...
        for i, encoder_block in enumerate(self.encoder_blocks):
            enc = encoder_block(enc, enc, enc, None, None, adj, self.geo_mask_dict[select_dataset].to(self.device), self.sem_mask)
