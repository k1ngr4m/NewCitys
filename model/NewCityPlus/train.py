...
def load_nyc_taxi_data(data_path):
    ...
    # 数据形状为 [time_steps, nodes, features]
    # 需要转换为 [nodes, time_steps, features] 以适应模型输入
    time_steps, nodes, features = raw_data.shape
    raw_data = np.transpose(raw_data, (1, 0, 2))  # 转换为 [nodes, time_steps, features]
    print(f"Transposed data shape: {raw_data.shape}")
    
    # 为了适应模型，我们需要重新组织数据为 [samples, time_steps, nodes, features]
    # 但我们只有 [nodes, time_steps, features]
    # 我们将每个节点视为一个独立的样本，或者创建时间序列样本
    
    # 创建序列数据 - 每个节点的时间序列
    input_window = 12  # 减小窗口大小以适应较短的时间序列
    output_window = 12
    stride = 1  # 减小步长
    
    xs, ys = [], []
    # 选择前几个节点进行训练以减少计算量
    num_nodes_to_use = min(100, nodes)  # 只使用前100个节点
    for node_idx in range(num_nodes_to_use):
        ...
        for i in range(0, time_steps - input_window - output_window, stride):
            ...
            # 扩展维度以适应模型输入 [1, time_steps, 1, features]
